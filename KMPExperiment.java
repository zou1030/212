    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int tSize : textSizes) {
            for (int pSize : patternSizes) {
                ...
                // Run the KMP matching multiple times for accuracy
                for (int i = 0; i < runs; i++) {
                    long start = System.nanoTime();           // Start timing
                    findKMP(textArr, patternArr);             // Run KMP
                    long end = System.nanoTime();             // End timing
                    totalElapsedNs += (end - start);          // Accumulate elapsed time
                }


                double avgElapsedMs = totalElapsedNs / 1000 / runs;


                writer.write(tSize + "," + pSize + "," + String.format("%.4f", avgElapsedMs) + "\n");


                System.out.printf("TextSize=%d, PatternSize=%d, AvgTime=%.4f ms\n", tSize, pSize, avgElapsedMs);
            }
        }
